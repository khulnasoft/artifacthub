## Inspektor gadgets repositories

Inspektor gadgets repositories are expected to be hosted in GitHub, GitLab or Bitbucket repos. When adding your repository to Artifact Hub, the url used **must** follow the following format:
...
The structure of a repository with multiple gadgets packages and versions could look something like this:
...
The [Artifact Hub metadata file](https://github.com/khulnasoft/artifacthub/blob/master/docs/metadata/artifacthub-pkg.yml) allows defining containers images. Inspektor gadgets packages are **required** to provide an image named `gadget` that points to the image containing the gadget. Packages can optionally provide an alternative location for the gadget image by defining a second image named `gadget-alternative-location`. Artifact Hub will check if the images provided have been signed with `cosign` and will consider the package to be *signed* when **all** images are signed.

Once you have added your repository, you are all set up. As you add new versions of your gadgets packages or new packages to your git repository, they'll be automatically indexed and listed in Artifact Hub.
